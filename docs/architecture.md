...
`lib/providers/finapi.service.ts` and `lib/providers/tigzig.service.ts` wrap every endpoint the app
...
request. `lib/catalog/benchmarkCatalog.ts` is server-only and contains only verified
total-return mappings. Both provider services use a 10-second timeout and
Next.js revalidation of 300 seconds.

**Neither provider service, nor `lib/providers/mongo.ts`, is imported from `app/`.**
Route handlers and pages only ever import one of two UI-facing facades:
`lib/catalog/catalog.service.ts` (discovery) or `lib/fund/fund.service.ts` (live per-scheme
...
`lib/catalog/catalog.service.ts` builds, persists, and serves the eligible-scheme
catalogue in MongoDB (`lib/providers/mongo.ts`). It is TigZig-primary:
...
FinAPI. See [lib/catalog/fundCatalog.types.ts](../lib/catalog/fundCatalog.types.ts) for the
full record shape.

## Fund service

`lib/fund/fund.service.ts` is the UI-facing facade for live, per-scheme lookups -
...
`lib/shared/deviceId.ts` issues an anonymous `nn_device_id` cookie (httpOnly,
...
`lib/watchlist/watchlist.service.ts` is Mongo-backed CRUD (collection `watchlists`,
see `lib/watchlist/watchlist.types.ts` for the document shape) exposed to routes under
`app/api/watchlists/`:
...
`lib/watchlist/watchlist.schema.ts` and `lib/watchlist/watchlist.client.ts` mirror the
...
`lib/fund/fund.types.ts` defines the internal `FundResearch` contract. Its
...
`lib/research/research-view/` converts that internal contract into semantic browser
...
`lib/fund/fund.schema.ts` defines Valibot schemas for browser-facing API responses.
`lib/fund/fund.client.ts` validates every client response against those schemas before
...
two-fund comparison. Shared calculations live in `lib/shared/analytics.ts`; use those
...
`MONGODB_URI` and `MONGODB_DB` (`lib/providers/mongo.ts`) configure the database behind
